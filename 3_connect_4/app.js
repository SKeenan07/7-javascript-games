document.addEventListener('DOMContentLoaded', () => {
    ...
    (function(index){
        ...
        squares[i].onclick = function(){
            // If the square below is taken, go on top of it
            if(squares[index + 7].classList.contains('taken')){
                ...
                // If the square below is empty, you can't go here
    ...
    function checkBoard() {
        ...
        // Take the 4 values and plug them into squares
        for(let y = 0; y < winningArrays.length; y++) {
            ...
            // Check if those arrays match Player 1
            if(square1.classList.contains('player-one') &&
            square2.classList.contains('player-one') &&
            square3.classList.contains('player-one') &&
            square4.classList.contains('player-one')) {
                // If yes, player one wins
                result.innerHTML = 'Player One wins!'
            }
            // Check if those arrays match Player 2
    ...
    // Add event listener to each square to trigger check board
